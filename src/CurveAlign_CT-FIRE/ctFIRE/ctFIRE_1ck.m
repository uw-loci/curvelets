function [OUTf OUTctf] = ctFIRE_1ck(imgPath,imgName,savePath,cP,ctfP)
...
%Feb, 2015: modified for the curvelet-knime integration 
...
fullname = fullfile(imgPath, imgName); % yl:use fullfile
...
if runCT == 1 %
    ...
    try
        ...
        if postp == 1%
            ...
            cP.RO = 1;  % for the individual mat file, make runORI = 0;  runCT = 1;
            save(fmat2,'data','Iname','p2','imgPath','imgName','savePath','cP','ctfP');
            disp(sprintf('CT-FIRE output data was saved as %s',fmat2)); % yl:for CK integration
            OUTctf = data;
        end
...
%         end
        ...
        if plotflag == 1 % overlay ctFIRE extracted fibers on the original image
            ...
            clrr2 = rand(LFa,3); % set random color
            gcf52 = figure(52);clf;
            set(gcf52,'name','ctFIRE output: overlaid image ','numbertitle','off','Visible','off')
            ...
            disp(sprintf('Overlaid image was saved as %s',fOL2)); % yl:for CK integration 
...
%             set(gcf52,'position',[(0.02*sw0+0.5*sh0) 0.1*sh0 0.75*sh0,0.75*sh0*pixh/pixw]);
        ...
        if plotflagnof == 1 % just show extracted fibers%
            ...
            set(gcf152,'name','ctFIRE output: extracted fibers ','numbertitle','off','Visible', 'off');
...
%             figure(gcf152);imshow(fNOL2); drawnow  % % yl:for CK integration 
...
%             set(gcf152,'position',[(0.02*sw0+0.5*sh0)+40 0.1*sh0+20 0.75*sh0,0.75*sh0*pixh/pixw]);
        ...
        X2L = FLout;        % length
        if lenHV
% yl: in ctfk, only output the csv file, do not display the histogram
%             inc = (max(FLout)-min(FLout))/bins;
%             edgesL = min(FLout):inc:max(FLout);
%             edges = edgesL;    % bin edges
