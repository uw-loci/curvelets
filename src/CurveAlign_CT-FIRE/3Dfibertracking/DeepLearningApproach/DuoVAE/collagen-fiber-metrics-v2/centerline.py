...
class CenterLine():
    ...
    def compute_fiber_feats(self, seg_length=4, smooth_sigma=2):
        ...
        """
        points_dist = lambda pt_0, pt_1: math.sqrt((pt_0.x-pt_1.x)**2 + (pt_0.y-pt_1.y)**2)
        segment_angle = lambda segment: math.atan2((segment.point_1.y-segment.point_0.y), (segment.point_1.x-segment.point_0.x))
        ...
        for region in line_regions:
            ...
            if len(fragments.geoms) < 1:
                fragments = [shape_line]
            else:
                fragments = [frag for frag in fragments.geoms if not frag.is_empty]
