...
## 2026-06-15 — Release readiness audit + action plan
...
| File | Purpose |
|---|---|
| `docs/audit_and_to_do_list.md` | Release readiness audit — originally generated by Antigravity CLI, extended by Gemini Code Assistant; reviewed and action plan added by Claude Code |

**Modified files**
| File | Change |
|---|---|
| `docs/audit_and_to_do_list.md` | Added §7 — 14-item release action plan across 6 phases (A–F) grounded in confirmed code inspection |

### Action plan summary (§7)
- **Phase A (Bug Fixes)**: three confirmed bugs — piecewise-linear sampling div-by-zero (`core/distributions.py:255`), `np.indices()` memory overhead (`rendering/raster_3d.py:117`), O(N²·S²) topology collision check (`generation/topology_3d.py:240`).
- **Phase B (Testing)**: add `pytest` + `pytest-qt` to dev deps; algorithmic unit tests for FMD geometry, Richards–Wolf PSF, distributions, topology; headless offscreen `conftest.py`; 60% coverage gate.
- **Phase C (Documentation)**: README quickstart + install guide, parameter reference (`docs/parameters.md`), math spec, developer setup guide.
- **Phase D (Distribution)**: argparse CLI in `main.py`, `[project.scripts]` entry point in `pyproject.toml`, guard GUI imports behind `[gui]` extra so bare `pip install .` works headless, bundled example JSON configs.
- **Phase E (CI/CD)**: GitHub Actions lint, test matrix (ubuntu + windows, Python 3.10 + 3.12), headless-only workflow, release-on-tag workflow.
- **Phase F (Future refactoring, post-v1.0)**: KDTree topology acceleration, tiled raster streaming, decouple `CanonicalSample` to standalone package, pytest migration, type annotation pass.

### Pending / next steps
- [ ] Create `release/v1.0` branch from `master`
- [ ] Fix piecewise-linear sampling div-by-zero (Phase A.1 — highest priority bug)
- [ ] Add `pytest` to dev deps and verify all 13 tests pass (Phase B.1)
- [ ] Write README.MD quickstart (Phase C.1)
- [ ] Implement argparse CLI and `[project.scripts]` entry point (Phase D.1–D.2)
