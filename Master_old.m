...
% img = imread('TAC-1a.jpg'); 
% img = imread('TACS-1b.tif'); 
% img = imread('TACS-1c.jpg'); 
% img = imread('TACS-2a.jpg'); 
% img = imread('TACS-2b.jpg'); 
% img = imread('TACS-2c.jpg'); 
...
%cd '/Users/doot4runner/Documents/Research/Curvelets/Finding Collagen'

%% Take the curvelet transform
C = fdct_wrapping(double(img),1);
...
% img_rec_all = ifdct_usfft(C,1);
% 
% figure; image(real(img_rec_all)); axis('image'); colormap(gray); title(['Curvelet Transformed Image']);
...
for xx = 1:length(C)
  for yy = 1:length(C{xx})
       ...
       figure;
       imagesc(real(img_rec)); axis('image'); colormap(gray); %title(['partial reconst. with zeroing out all coeffs. but at C' num2str(xx) ',' num2str(yy)]);
       drawnow
