...
function [] = CAroi(CApathname,CAfilename,CAdatacurrent,CAcontrol)
        ...
        set(image_fig,'KeyPressFcn',@roi_mang_keypress_fn);                 %Assigning the function that is called when any key is pressed while image_fig is active
...
%                 end
    ...
    function[]=save_roi(object,handles)
        % searching for the biggest operation number
        finalize_rois=1;
        roi=getPosition(h);
        count=1;count_max=1;
        if(isempty(separate_rois)==0)
        ...
        %saving date and time of operation
        c=clock;fix(c);
        ...
        %index_temp includes the index numbers of selected ROIs and the
        %index of newly saved ROI
        ...
        display(index_temp);%Use index_temp to show the ROIs 
    end
...
%--------------------------------------------------------------------------
    ...
    function[]=load_roi_fn(object,handles)
        try 
            ...
            if(combined_rois_present==0)
                % for one ROI
                new_roi=[];
                active_filename=filename_temp; %format- testIMGdata1_ROI1_coordinates.txt
    ...
    function[]=display_rois(indices)
       ...
       %numbers would be taken not from uitable but as indices
        ...
        figure(image_fig);%imshow(IMGdata);
        warning('off');
        ...
        if(combined_rois_present==0) 
            xmid=[];ymid=[];
               ...
               overlaid_IMGdata(1:s1,1:s2,1)=IMGdata(1:s1,1:s2);overlaid_IMGdata(1:s1,1:s2,2)=IMGdata(1:s1,1:s2);
               Data=get(roi_table,'Data');
