...
% On August 27 2015,CTFroi took the current function name.
   ...
   if nargin == 0
       ...
       separate_rois = [];
       CTFroi_data_current = [];
   elseif nargin == 1 % Called from ctFIRE
       ...
       if exist(fullfile(CTFpathname,'ROI','ROI_management',sprintf('%s_ROIsCTF.mat',filenameNE)),'file')
             %separate_rois and CTFroi_data_current read from the file
      ...
      %Loading which slice is selectedin a stack, number of sections  in
      %the stack. And if not stack then using default values of 1 each
    ...
    global roi_anly_fig; roi_anly_fig=-1;
    global pseudo_address;              %Last used address -dont change the variable name 
    global image;
    ...
    uicontrol('Parent',roi_mang_fig,'Style','Pushbutton','Units','normalized','Position',[0.75 0.96 0.2 0.03],'String','Reset','Callback',@reset_fn,'TooltipString','Press to reset');
    ...
    uicontrol('Parent',roi_mang_fig,'Style','text','string','Draw ROI Menu (d)','Units','normalized','Position',[0.55 0.86 0.4 0.035]);
    ...
    uicontrol('Parent',roi_mang_fig,'Style','Pushbutton','Units','normalized','Position',[0.55 0.58 0.4 0.035],'String','Load ROI','TooltipString','Loads ROIs of other images','Enable','on','Callback',@text_to_roi_fn);
    ...
    uicontrol('Parent',roi_mang_fig,'Style','Text','Units','normalized','Position',[0.6 0.31 0.3 0.045],'String','Show All ROIs');
    
    index_box=uicontrol('Parent',roi_mang_fig,'Style','Checkbox','Units','normalized','Position',[0.55 0.27 0.1 0.045],'Callback',@index_fn);
    uicontrol('Parent',roi_mang_fig,'Style','Text','Units','normalized','Position',[0.6 0.26 0.28 0.045],'String','Show Indices');
    
    uicontrol('Parent',roi_mang_fig,'Style','text','Units','normalized','Position',[0.55 0.23 0.4 0.045],'String','Message');
    ...
    uicontrol('Parent',CTFroi_table_fig,'Style','Pushbutton','Units','normalized','Position',[0.9 0.01 0.08 0.08],'String','Delete','Callback',@DeleteROIout_Callback);
    uicontrol('Parent',CTFroi_table_fig,'Style','Pushbutton','Units','normalized','Position',[0.80 0.01 0.08 0.08],'String','Save All','Callback',@SaveROIout_Callback);
...
%output table callback functions
    function cleanupFn(~,~)
        if(isempty(h)~=1&&isvalid(h)), delete(h),end
...
% load_ctfimage funcction is based on the load_image function
    ...
    function load_ctfimage(~,~)
        ...
        try
            ...
            if(exist(fullfile(pathname,'ROI'),'dir')==0)%check for ROI folder
                ...
                mkdir(fullfile(pathname,'ROI','ROI_management','ctFIRE_on_ROI','ctFIREout'));
            else
            ...
            [~,filename,format]=fileparts(filename);
            ...
            Data=cell(size_saved_opeations,1);
    ...
    function[]=roi_mang_keypress_fn(~,eventdata,~)
        ...
        elseif(eventdata.Key=='x')
            delete(h);
            set(status_message,'String','Draw cancelled');
        end
    end
    ...
    function[]=load_image(~,~)
        [filename,pathname,~]=uigetfile({'*.tif';'*.tiff';'*.jpg';'*.jpeg'},'Select image',pseudo_address,'MultiSelect','off');
...
% Function to draw ROIs - the only one that should be there
    function[]=roi_shape_choice_fn(~,~)
        if(isa(h,'imrect')||isa(h,'imellipse')||isa(h,'imfreehand')||isa(h,'impoly'))
           delete(h); 
        end
        set(save_roi_box,'Enable','on');
        ...
        elseif(roi_shape_temp==6)
            set(status_message,'String','change position of rectangle');
        end
        figure(image_fig);
        ...
        set(status_message,'String','press s to save and x to cancel');
        wait(h);
        ...
        function[]=roi_shape_popup_window()
            ...
            defaultBackground = get(0,'defaultUicontrolBackgroundColor');
            popup_new_roi=figure('Units','pixels','Position',[round(ScreenWidth*0.05) round(0.65*SHeight)  200 100],'Menubar','none','NumberTitle','off','Name','Select ROI shape','Visible','on','Color',defaultBackground);
            
            uicontrol('Parent',popup_new_roi,'Style','text','string','Fixed Size Rect ROI','Units','normalized','Position',[0.15 0.8 0.6 0.15]);
            uicontrol('Parent',popup_new_roi,'Style','edit','Units','normalized','String',num2str(height),'Position',[0.05 0.5 0.2 0.15],'enable','on','Callback',@rect_roi_height_fn);
            uicontrol('Parent',popup_new_roi,'Style','text','string','Height','Units','normalized','Position',[0.28 0.5 0.2 0.15],'enable','on');
            uicontrol('Parent',popup_new_roi,'Style','edit','Units','normalized','String',num2str(width),'Position',[0.52 0.5 0.2 0.15],'enable','on','Callback',@rect_roi_width_fn);
            uicontrol('Parent',popup_new_roi,'Style','text','string','Width','Units','normalized','Position',[0.73 0.5 0.2 0.15],'enable','on');
            
            uicontrol('Parent',popup_new_roi,'Style','edit','Units','normalized','String',num2str(x),'Position',[0.05 0.3 0.2 0.15],'enable','on','Callback',@x_change_fn);
            uicontrol('Parent',popup_new_roi,'Style','text','string','ROI X','Units','normalized','Position',[0.28 0.3 0.2 0.15],'enable','on');
            uicontrol('Parent',popup_new_roi,'Style','edit','Units','normalized','String',num2str(y),'Position',[0.52 0.3 0.2 0.15],'enable','on','Callback',@y_change_fn);
            uicontrol('Parent',popup_new_roi,'Style','text','string','ROI Y','Units','normalized','Position',[0.73 0.3 0.2 0.15],'enable','on');
            
            uicontrol('Parent',popup_new_roi,'Style','pushbutton','string','Ok','Units','normalized','Position',[0.05 0.10 0.45 0.2],'Callback',@ok_fn,'Enable','on');
            
            function[]=rect_roi_width_fn(object,~)
                width=str2doule(get(object,'string'));
            end
            
            function[]=rect_roi_height_fn(object,~)
                height=str2double(get(object,'string'));
            end
            ...
            function[]=x_change_fn(object,~)
                x=str2double(get(object,'string'));
                %display(x);
            end
            
            function[]=y_change_fn(object,~)
                y=str2double(get(object,'string'));
                %display(y);
            end
        end
    ...
    function[]=save_roi(~,~)
        %only save simple ROIs
        roi=getPosition(h);
        count=1;
        count_max=1;
        if(isempty(separate_rois)==0)
            while(count<MAXPOSSIBLEROIS)
        ...
        if(iscell(roi_shape)==0)
            %display('single ROI');
            if(roi_shape==1)%Rectangle
                roiDataCopy=roi;
                a=roiDataCopy(1);b=roiDataCopy(2);c=roiDataCopy(3);d=roiDataCopy(4);
                vertices(:,:)=[a,b;a+c,b;a+c,b+d;a,b+d;];
                BW=roipoly(image,vertices(:,1),vertices(:,2));
                x_min=floor(a);
                x_max=floor(a+c);
                ...
                d=data2(4);
                vertices(:,:)=[a,b;a+c,b;a+c,b+d;a,b+d;];
                separate_rois.(fieldname).roi=vertices;
            ...
            elseif(roi_shape==3)
                ...
                a=roiDataCopy(1);b=roiDataCopy(2);c=roiDataCopy(3);d=roiDataCopy(4);
                s1=size(image,1);s2=size(image,2);
                BW=logical(zeros(s1,s2)); %#ok<LOGL>
                for m=1:s1
                    for n=1:s2
                        dist=(n-(a+c/2))^2/(c/2)^2+(m-(b+d/2))^2/(d/2)^2;%Ellipse equation 
                        if(dist<=1.00)
                            BW(m,n)=true;
                        else
                            BW(m,n)=false;
                ...
                boundary=boundary{1};
                separate_rois.(fieldname).roi=[boundary(:,2) boundary(:,1)];
...
%         2 combining combined ROIs (also includes individual ROIs)
        ...
        %Formulating the Combined ROI name
        combined_roi_name=[];
        for i=1:s1
           if(i==1)
        ...
        % this loop adds the component ROIs to the combined ROI
        if(combined_rois_present==0)%all of the rois to be combined are not a combination of rois.
            for i=1:s1
                ...
                separate_rois.(combined_roi_name).enclosing_rect{i}=separate_rois.(roi_names{cell_selection_data(i,1),1}).enclosing_rect; 
            end
        else %atleast one ROI is a combined ROI itself
            count=1;
            for i=1:s1
                if(iscell(separate_rois.(roi_names{cell_selection_data(i,1),1}).shape)==0)
                %if a ROI has its shape as a cell then it is a combined ROI
                    ...
                    separate_rois.(combined_roi_name).enclosing_rect{count}=separate_rois.(roi_names{cell_selection_data(i,1),1}).enclosing_rect;
                    count=count+1;
                else
                %if a ROI has its shape as a cell then it is a combined ROI - appending all of the components of ROI to the final combined ROI
                    ...
                    for j=1:stemp
                        ...
                        separate_rois.(combined_roi_name).enclosing_rect{count}=separate_rois.(roi_names{cell_selection_data(i,1),1}).enclosing_rect{j};
                        count=count+1;
                    end
                end
            end
    ...
    function[]=update_rois
        %Function to update the rois in the ui table
        separate_rois=importdata(fullfile(pathname,'ROI','ROI_management',[filename,'_ROIs.mat']));
        size_saved_operations=size(fieldnames(separate_rois),1);
        Data=cell(size_saved_operations,1);
    ...
    function[]=cell_selection_fn(~,handles)
        ...
        %Button controls when zero,one or multiple ROIs are selected
        if(stemp>1)
        ...
        if(stemp>=1)
           set([delete_roi_box,measure_roi_box,save_roi_text_box,save_roi_mask_box,analyzer_box],'Enable','on');
        else
            set([delete_roi_box,measure_roi_box,save_roi_text_box,save_roi_mask_box,analyzer_box],'Enable','off');
        end
        ...
        combined_rois_present=0; 
         ...
         if(combined_rois_present==0)
             for k=1:s3
                 ...
                 figure(image_fig);
                 vertexTemp=vertices;
                 vertexTemp(end+1,:)=vertexTemp(1,:);
                 plot(vertexTemp(:,1),vertexTemp(:,2), 'y', 'LineWidth', 2);
         ...
         elseif(combined_rois_present==1)
             ...
             for k=1:s3
                 if (iscell(separate_rois.(Data{handles.Indices(k,1),1}).roi)==1)
                     ...
                     xmid_temp=cell(s_subcomps);
                     ymid_temp=cell(s_subcomps);
                     for p=1:s_subcomps
                         xmid_temp{p}=separate_rois.(Data{handles.Indices(k,1),1}).xm{p};
                         ymid_temp{p}=separate_rois.(Data{handles.Indices(k,1),1}).ym{p};
                     ...
                     xmid{k}=xmid_temp; 
                     ymid{k}=ymid_temp;
                 elseif (iscell(separate_rois.(Data{handles.Indices(k,1),1}).roi)==0)
                     vertices=separate_rois.(Data{handles.Indices(k,1),1}).roi;
                     figure(image_fig);
                     vertexTemp=vertices;
                     vertexTemp(end+1,:)=vertexTemp(1,:);
         ...
         if(get(index_box,'Value')==1)
             for k=1:s3
                 %Green Color text for combined ROIs and yellow for
                 %individual ROIs
                 if(iscell(xmid{k})==1)
                     subcomps=size(xmid{k});
                     for i=1:subcomps
                         figure(image_fig);ROI_text(k)=text(ymid{k}{i},xmid{k}{i},Data{cell_selection_data(k,1),1},'HorizontalAlignment','center','color',[0 1 0]);hold on; 
                     end
                 else
    ...
    function[]=rename_roi(~,~)
        ...
        uicontrol('Parent',rename_roi_popup,'Style','text','Units','normalized','Position',[0.05 0.75 0.9 0.2],'String','Enter the new name below','BackgroundColor',defaultBackground);
        ...
        uicontrol('Parent',rename_roi_popup,'Style','Pushbutton','Units','normalized','Position',[0.5 0.05 0.4 0.2],'String','Ok','BackgroundColor',defaultBackground,'Callback',@ok_fn);
        ...
        function[]=ok_fn(~,~)
           ...
           if(new_fieldname_present==0)
               ...
               uicontrol('Parent',error_figure,'Style','text','Units','normalized','Position',[0.05 0.05 0.9 0.9],'String','Error-Name Already Exists','ForegroundColor',[1 0 0],'FontSize',15);
               pause(2);
               close(error_figure);
           end
        end
    ...
    function[]=delete_roi(~,~)
        temp_fieldnames=fieldnames(separate_rois);
        message='ROI ';endmessage=' is(are) deleted';
    ...
    function[]=measure_roi(~,~)
       ...
       s3=size(cell_selection_data,1);
        ...
        measure_data{1,1}='Names';
        measure_data{1,2}='Min pixel value';
        measure_data{1,3}='Max pixel value';
        measure_data{1,4}='Area';
        measure_data{1,5}='Mean pixel value';
       for k=1:s3
          ...
          elseif (iscell(separate_rois.(Data{cell_selection_data(k,1),1}).roi)==1)
              ...
              for m=1:s_subcomps
                  ...
                  BWtemp=roipoly(image,vertices(:,1),vertices(:,2));
                  BW=BW|BWtemp;
              end
          end
          [minVal,maxVal,area,mean]=roi_stats(BW);
          measure_data{k+1,1}=Data{cell_selection_data(k,1),1};
          measure_data{k+1,2}=minVal;
          measure_data{k+1,3}=maxVal;
     ...
     function[minVal,maxVal,area,meanVal]=roi_stats(BW)
        imageTemp=image(:,:,1).*uint8(BW);
        minVal=min(min(imageTemp(:)));
        maxVal=max(max(imageTemp(:)));
        area=sum(sum(BW(:)));
        meanVal=sum(sum(imageTemp))/area;
     end
    ...
    function [] = mask_to_roi_fn(~,~)
        ...
        if(size(mask_image,3)==3)
           mask_image=rgb2gray(mask_image);
        end
        if(~isa(mask_image,'logical'))
            mask_image=floor(mask_image/max(max(mask_image(:))));%binarizes the image
        end
        
        boundaries=bwboundaries(mask_image);
        for i=1:size(boundaries,1)
            boundaries_temp=boundaries{i,1};
            %Adding the ROI to separate_rois variable which is saved later
        ...
        function[]=mask_to_roi_sub_fn(boundaries)
            if(isfield(separate_rois,'imported_maskROI1'))
               ...
               fieldname= 'imported_maskROI1';
                ...
                time=[num2str(c(4)) ':' num2str(c(5)) ':' num2str(uint8(c(6)))]; % saves 11:50:32 for 1150 hrs and 32 seconds
                separate_rois.(fieldname).time=time;
                %setting the roi_shape to be Freehand
                ...
                BW=roipoly(image,boundaries(:,2),boundaries(:,1));
                [xm,ym]=midpoint_fn(transpose(BW));
                separate_rois.(fieldname).xm=xm;
                separate_rois.(fieldname).ym=ym;
    ...
    %Checked till here 10th October - 6:30 pm  
...
%         display(roi_anly_fig);
        if(roi_anly_fig<=0)
            roi_anly_fig = figure('Resize','off','Color',defaultBackground,'Units','pixels','Position',[50+round(ScreenWidth/5)+relative_horz_displacement 0.7*SHeight-65 round(ScreenWidth/10*1) round(SHeight*0.35)],'Visible','on','MenuBar','none','name','ROI Analyzer','NumberTitle','off','UserData',0);
        else
        ...
        function[]=plot_statisitcs_fn(handles,object)
            ...
            statistics_fig = figure('Resize','on','Color',defaultBackground,'Units','pixels','Position',[50+round(ScreenWidth/10*3.1)+relative_horz_displacement 50 round(ScreenWidth/10*6.3) round(SHeight*0.85)],'Visible','on','name','ROI Manager','UserData',0);
...
%                    hold off;
        ...
        function[]=more_settings_fn(object,handles)
            ...
            settings_position_vector=[50+round(ScreenWidth/5*1.5)+relative_horz_displacement_settings SHeight-190 260 160];
        ...
        position_vector=[50+round(ScreenWidth/5*1.5)+50 SHeight-260 260 90];
...
%                %display(type);
    ...
    function[]=text_to_roi_fn(~,~)
        ...
        try
            ...
            set(status_message,'String','Error in loading text coordinates');
            return;
        end
        ...
        if(combined_rois_present==0)
            ...
            roi_temp=zeros(vertex_size,2);
            temp1=fscanf(fileID,'%f ');
            roi_temp(:,1)=temp1(1:2:end);
            roi_temp(:,2)=temp1(2:2:end);
            count=1;count_max=1;
            if(isempty(separate_rois)==0)
               while(count<1000)
                  fieldname=['ROI' num2str(count)];
            ...
            separate_rois.(fieldname).date=date;
            separate_rois.(fieldname).time=time;
            separate_rois.(fieldname).shape=str2double(shape);
            [xm,ym]=midpoint_fn(roipoly(image,roi_temp(:,1),roi_temp(:,2)));
            separate_rois.(fieldname).xm=xm;
            separate_rois.(fieldname).ym=ym;
            [x_min,y_min,x_max,y_max]=enclosing_rect(roi_temp);
            enclosing_rect_values=[x_min,y_min,x_max,y_max]; 
            separate_rois.(fieldname).enclosing_rect=enclosing_rect_values;
            save(fullfile(pathname,'ROI','ROI_management',[filename,'_ROIs.mat']),'separate_rois','-append'); 
            update_rois;
        elseif(combined_rois_present==1)
            ...
            if(isempty(separate_rois)==0)
               while(count<1000)
                  fieldname=['combined_ROI_' num2str(count)];
                   if(isfield(separate_rois,fieldname)==1)
                      count_max=count;
                   end
                  count=count+1;
               end
               fieldname=['combined_ROI_' num2str(count_max)];
            else
               fieldname='combined_ROI_1';
            end
            ...
            for k=1:total_rois_number
                ...
                for i=1:vertex_size
                    temp1=fgetl(fileID);
                    roi_temp(i,:)=floor(sscanf(temp1,'%f %f'));
                end
                separate_rois.(fieldname).roi{k}=roi_temp;
                separate_rois.(fieldname).date=date;
                separate_rois.(fieldname).time=time;
                separate_rois.(fieldname).shape{k}=str2double(shape);
                [xm,ym]=midpoint_fn(roipoly(image,roi_temp(:,1),roi_temp(:,2)));
                separate_rois.(fieldname).xm{k}=xm;
                separate_rois.(fieldname).ym{k}=ym;
                [x_min,y_min,x_max,y_max]=enclosing_rect(roi_temp);
                enclosing_rect_values=[x_min,y_min,x_max,y_max];
                separate_rois.(fieldname).enclosing_rect{k}=enclosing_rect_values;
            end
            save(fullfile(pathname,'ROI','ROI_management',[filename,'_ROIs.mat']),'separate_rois','-append'); 
            update_rois;
        end
         ...
         elseif(combined_rois_present==1)
             ...
             for k=1:stemp
                 if (iscell(separate_rois.(Data{indices(k),1}).roi)==1)
                     combined_name_for_ctFIRE=[combined_name_for_ctFIRE '_' Data{indices(k),1}];
                     s_subcomps=size(separate_rois.(Data{indices(k),1}).roi,2);
                     xmid_temp=cell(s_subcomps);
                     ymid_temp=cell(s_subcomps);
                     for p=1:s_subcomps
                         xmid_temp{p}=separate_rois.(Data{indices(k),1}).xm{p};
                         ymid_temp{p}=separate_rois.(Data{indices(k),1}).ym{p};
                     ...
                     xmid{k}=xmid_temp; 
                     ymid{k}=ymid_temp;
                 elseif (iscell(separate_rois.(Data{indices(k),1}).roi)==0)
                     vertices=separate_rois.(Data{indices(k),1}).roi;
                     figure(image_fig);
                     vertexTemp=vertices;
                     vertexTemp(end+1,:)=vertexTemp(1,:);
         ...
         if(get(index_box,'Value')==1)
             for k=1:stemp
                 ...
                 if(iscell(xmid{k})==1)
                     subcomps=size(xmid{k});
                     for i=1:subcomps
                         figure(image_fig);ROI_text(k)=text(ymid{k}{i},xmid{k}{i},Data{indices(k),1},'HorizontalAlignment','center','color',[0 1 0]);hold on; 
                     end
                 else
