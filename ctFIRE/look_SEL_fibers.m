...
if cP.stack == 0
    ...
    if plotflag
        ...
        gcf1 = figure('Resize','on','Units','pixels','Position',[225 250 512 512],'Visible','on',...
            'MenuBar','none','name','Overlaid selected fibers','NumberTitle','off','UserData',0);
    ...
    if lenHV
        ...
        gcf = figure(212); clf
        set(gcf,'name',sprintf('length:%s',imgName),'numbertitle','off')
        set(gcf,'position',[0.60*sw0 0.25*sh0 0.35*sh0,0.20*sh0])
    ...
    if angHV
        ...
        gcf = figure(213); clf
        set(gcf,'name',sprintf('angle:%s',imgName),'numbertitle','off')
        set(gcf,'position',[(0.60*sw0+0.35*sh0) 0.25*sh0 0.35*sh0,0.20*sh0])
    ...
    if strHV
        ...
        gcf = figure(214); clf
        set(gcf,'name',sprintf('straigthness:%s',imgName),'numbertitle','off')
        set(gcf,'position',[(0.375*sw0+0.05*sh0) 0.25*sh0 0.35*sh0,0.20*sh0])
    ...
    if widHV == 1
        ...
        gcf = figure(211); clf
        set(gcf,'name',sprintf('Width:%s',imgName),'numbertitle','off')
        set(gcf,'position',[(0.175*sw0+0.05*sh0) 0.25*sh0 0.35*sh0,0.20*sh0])
...
elseif cP.stack == 1
    ...
    if MAC == 1
        ...
        xlwrite(fullfile(selPath,selName),sheetTab,'Combined ALL','A1');
        xlwrite(fullfile(selPath,selName),fiberall,'Combined ALL','B2');
    else
        xlswrite(fullfile(selPath,selName),sheetTab,'Combined ALL','A1');
        xlswrite(fullfile(selPath,selName),fiberall,'Combined ALL','B2');
    end
    ...
    if plotflag
        ...
        for i = 1:length(imgName)
            OLList = dir([selPath,imgName{i},'*.tif']);
            gcf1 = figure('Resize','on','Units','pixels','Position',[225+20*i 250+15*i 512 512],'Visible','on',...
                'MenuBar','none','name',sprintf('Overlaid selected fibers,image%d,%s',i,imgName{i}),'NumberTitle','off','UserData',0);
            imshow([selPath OLList.name]); axis equal image;
        end
        ...
        if lenHV
            ...
            edges = edgesL;    % bin edges
            gcf = figure(302); clf
            set(gcf,'name',sprintf('lenHIST:%s',selName),'numbertitle','off')
            set(gcf,'position',[0.60*sw0 0.55*sh0 0.35*sh0,0.35*sh0])
            [NL,BinL] = histc(X2L,edges);
            bar(edges,NL,'histc');
            xlim([min(FLout) max(FLout)]);
            ...
            %
            gcf = figure(312); clf
            set(gcf,'name',sprintf('length,combined:%s',selName),'numbertitle','off')
            set(gcf,'position',[0.60*sw0 0.25*sh0 0.35*sh0,0.20*sh0])
        ...
        if angHV
            ...
            edges = edgesA;    % bin edges
            gcf = figure(303); clf
            set(gcf,'name',sprintf('angHIST:%s',selName),'numbertitle','off')
            set(gcf,'position',[(0.60*sw0+0.35*sh0) 0.55*sh0 0.35*sh0,0.35*sh0])
            [NA,BinA] = histc(X2A,edges);
            bar(edges,NA,'histc');
            axis square
            % YLtemp           title(sprintf('Extracted angle hist'),'fontsize',fz1);
            ...
            gcf = figure(313); clf
            set(gcf,'name',sprintf('angle,combined:%s',selName),'numbertitle','off')
            set(gcf,'position',[(0.60*sw0+0.35*sh0) 0.25*sh0 0.35*sh0,0.20*sh0])
        ...
        if strHV
            ...
            gcf = figure(314); clf
            set(gcf,'name',sprintf('straigthness,combined:%s',selName),'numbertitle','off')
            set(gcf,'position',[(0.375*sw0+0.05*sh0) 0.25*sh0 0.35*sh0,0.20*sh0])
        ...
        if widHV == 1
            ...
            set(gcf,'name',sprintf('Width,combined:%s',selName),'numbertitle','off')
            set(gcf,'position',[(0.175*sw0+0.05*sh0) 0.25*sh0 0.35*sh0,0.20*sh0])
