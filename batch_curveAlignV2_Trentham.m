...
function batch_curveAlignV2_Trentham()
...
if bdryTest
    ...
    for i = 1:numFiles
        ...
        if ~isempty(regexp(bdryFiles(i).name,'normal', 'once', 'ignorecase'))
            imFileName = [bdryFiles(i).name(14:end-4) '.tif'];
        else
            imFileName = ['DCIS ' bdryFiles(i).name(14:end-4) '.tif'];
        end        
        for j = 1:lenFileList
    ...
    disp(['Found ' int2str(numFiles) ' boundary files and ' int2str(imgNum) ' image files.']);
else
...
if numFiles == 0
    errordlg('Directory does not contain valid images or boundaries.');
