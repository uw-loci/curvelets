function[X F V R] = reduce_network(X,F,V,R,thresh_merge,scale)
%REDUCE_NETWORK - reduces fiber network 1) keeping only the fiber ends and
%crosslinks, 2) merging fibers together over pretty long distances

%remove network ends and crosslinks
    for fi=1:length(F)
        v = F(fi).v;
        l = zeros(size(v));        
        l(1) = Inf;
...
%merge vertices
    X10 = ceil(X/scale(1));
    thresh_merge = thresh_merge/scale(1);
    for i=4:4:ceil(thresh_merge)
        fprintf('%d, ',i); 
        X10 = round(X10);
        [X10 F V R] = fiber_mergevert(X10,F,V,R,fliplr(max(X10)),i,0);  %when two points are close together and pointing in the same direction, but on different fibers, they should be merged into a single point
        [X10 F V R] = remove_repeat(X10,F,V,R);    
    end
    X = X10*scale(1);
